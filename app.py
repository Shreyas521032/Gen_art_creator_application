...
def generate_functions():
    ...
    function_type1 = st.session_state.get('function_type1', 'sin')
    function_type2 = st.session_state.get('function_type2', 'cos')
    
    if function_type1 == 'custom':
        custom_function1 = st.session_state.get('custom_function1', '')
        try:
            def f1(x, y):
        ...
        operation1 = st.session_state.get('operation1', '+')
    ...
    if function_type2 == 'custom':
        custom_function2 = st.session_state.get('custom_function2', '')
        try:
            def f2(x, y):
        ...
        operation2 = st.session_state.get('operation2', '*')
...
def create_art():
    seed = st.session_state.get('seed', random.randint(1, 100000))
    color = st.session_state.get('color', 'viridis')
    projection = st.session_state.get('projection', 'rectilinear')
    ...
    if st.session_state.get('apply_random_sampling', False):
        g.sample_random(st.session_state.get('random_sampling', 5000))
    
    if st.session_state.get('apply_random_color', False):
        g.random_color()
    else:
        g.set_color(color)
    ...
    # Apply other filters
    if st.session_state.get('apply_gradient', False):
        alpha = st.session_state.get('alpha', 0.5)
        g.set_alpha(alpha)
    
    fig = plt.figure(figsize=(st.session_state.get('width', 12), 
                              st.session_state.get('height', 10)))
...
with st.sidebar:
    ...
    if use_random_seed:
        ...
        st.session_state['seed'] = st.number_input("Seed", 
                                                   min_value=1, 
                                                   max_value=100000, 
    ...
    # Color options
    st.session_state['apply_random_color'] = st.checkbox("Use Random Colors", value=False, key='apply_random_color')
    
    if not st.session_state['apply_random_color']:
        st.session_state['color'] = st.selectbox("Color Palette", sorted(VALID_COLORS), index=sorted(VALID_COLORS).index('viridis') if 'viridis' in VALID_COLORS else 0)
    
    # Projection options
    projection_options = ["None", "rectilinear", "polar", "aitoff", "hammer", "lambert", "mollweide"]
    st.session_state['projection'] = st.selectbox("Projection", projection_options, index=0)
    ...
    cols = st.columns(2)
    with cols[0]:
        st.session_state['width'] = st.slider("Width", 5, 20, 12)
    with cols[1]:
        st.session_state['height'] = st.slider("Height", 5, 20, 10)
    
    # Additional effects
    st.session_state['apply_gradient'] = st.checkbox("Apply Gradient", value=True)
    if st.session_state['apply_gradient']:
        st.session_state['alpha'] = st.slider("Transparency", 0.1, 1.0, 0.5)
    ...
    if st.session_state['apply_random_sampling']:
        st.session_state['random_sampling'] = st.slider("Sample Points", 1000, 10000, 5000)
...
with col2:
    if 'generate_pressed' in st.session_state:
        ...
        st.write(f"**Seed:** {g.seed}")
        
        # Show function equations
        st.write("**Functions used:**")
        
        if st.session_state['function_type1'] == 'custom':
            st.code(f"f1(x,y) = {st.session_state.get('custom_function1', 'sin(x)*cos(y)')}")
        else:
            operation = st.session_state.get('operation1', '+')
            func = st.session_state['function_type1']
            st.code(f"f1(x,y) = {func}(x) {operation} {func}(y)")
        
        if st.session_state['function_type2'] == 'custom':
            st.code(f"f2(x,y) = {st.session_state.get('custom_function2', 'cos(x)*sin(y)')}")
        else:
            operation = st.session_state.get('operation2', '*')
            func = st.session_state['function_type2']
            st.code(f"f2(x,y) = {func}(x) {operation} {func}(y)")
        
        # Show other parameters
        st.write(f"**Color Palette:** {st.session_state.get('color', 'Random' if st.session_state.get('apply_random_color', False) else 'viridis')}")
        st.write(f"**Projection:** {st.session_state.get('projection', 'None')}")
        
        if st.session_state.get('apply_gradient', False):
            st.write(f"**Transparency:** {st.session_state.get('alpha', 0.5)}")
        
        if st.session_state.get('apply_random_sampling', False):
            st.write(f"**Random Sampling Points:** {st.session_state.get('random_sampling', 5000)}")
...
# Footer
st.markdown("---")
st.markdown("<p style='text-align: center'>Made with ❤️ by AI Art Enthusiasts | 2025</p>", unsafe_allow_html=True)
